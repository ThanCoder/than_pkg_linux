...
than_pkg_linux_plugin_handle_method_call(ThanPkgLinuxPlugin *self,
  ...
  // Launcher
    ...
    response = launch_url(fl_method_call_get_args(method_call));
  } else if (strcmp(method, "launch_path") == 0) {
    response = launch_path(fl_method_call_get_args(method_call));
  } else if (strcmp(method, "launch_app") == 0) {
    response = launch_app(fl_method_call_get_args(method_call));
...
/********************* Launcher Funcs **************************** */
/********************* Pure GLib Standard Launcher Funcs
 * **************************** */
